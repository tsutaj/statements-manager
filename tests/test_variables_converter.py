...
class TestToString:
    def test_to_string_non_custom_comma_separator(self):
        ...
        result = to_string(1000, config, is_custom_output=False)
        assert result == "1{,}000"
    ...
    def test_to_string_custom_comma_separator(self):
        config = MockStatementConfig(digit_separator=",")
        result = to_string(1000, config, is_custom_output=True)
        assert result == "1,000"
    ...
    def test_to_string_exponential_notation(self):
        ...
        result = to_string(1000000, config, is_custom_output=False)
        assert result == "10^{6}"

    def test_to_string_exponential_notation_custom(self):
        config = MockStatementConfig(exponential_threshold=1000)
        result = to_string(1000000, config, is_custom_output=True)
        assert result == "10^{6}"

    def test_to_string_non_integer(self):
        config = MockStatementConfig()
        result = to_string("test", config, is_custom_output=False)
        assert result == "test"

    def test_to_string_non_integer_custom(self):
        config = MockStatementConfig()
        result = to_string("test", config, is_custom_output=True)
        assert result == "test"
